...
- **Agent-agnostic**: works with Claude Code, Codex, Rovo Dev, OpenCode, GitHub Copilot CLI, Pi, Command Code, or ACP targets out of the box
...
- Flags that `gnhf` manages itself for a given agent, such as output-shaping or local-server startup flags, are rejected during config loading so you get a clear error instead of duplicate-argument ambiguity. For `cursor` and `pi` specifically, secret-bearing auth flags such as `--api-key` and Cursor's `--auth-token` are also blocked; configure Cursor with `CURSOR_API_KEY` or `CURSOR_AUTH_TOKEN` and Pi via Pi's own config or the environment variable it reads, not via `agentArgsOverride`.
...
For Cursor, `gnhf` prefers the editor launcher form (`cursor agent`) only when the `cursor` binary exposes the `agent` subcommand, then falls back to standalone Cursor Agent CLI binaries (`cursor-agent` or `agent`) only when their help identifies them as Cursor Agent. You can still set `agentPathOverride.cursor` to `agent`, `cursor-agent`, or a compatible wrapper; `gnhf` invokes known standalone binary names directly and also detects renamed standalone wrappers from their `--help` output.
...
| Command Code       | `--agent commandcode`             | Install Command Code (`npm i -g command-code`) and log in with `cmd login` first.                                                                                                   | `gnhf` invokes `command-code` in headless `-p` print mode with `--trust`, `--skip-onboarding`, `--yolo`, and `--max-turns 30`. It embeds the output schema in the prompt and parses structured JSON from stdout. Pass your own `--max-turns` or `--add-dir` to adjust the turn limit or add extra workspace directories. The CLI reports no token usage, so gnhf estimates it (totals render with a `~` prefix) and `--max-tokens` still applies.                                                                                                                                                                      |
