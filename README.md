...
| Command Code       | `--agent commandcode`             | Install Command Code (`npm i -g command-code`) and log in with `cmd login` first.                                                                                                   | `gnhf` invokes `command-code` in headless `-p` print mode with `--trust`, `--skip-onboarding`, `--yolo`, and `--max-turns 30`. It embeds the output schema in the prompt and parses structured JSON from stdout. Pass your own `--plan`, `--permission-mode`, `--max-turns`, or `--yolo` to manage execution permissions or turn limits. Token usage is not reported by the CLI.                                                                                                                                                                      |
