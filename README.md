...
| Command Code       | `--agent commandcode`             | Install Command Code (`npm i -g command-code`) and log in with `cmd login` first.                                                                                                   | `gnhf` invokes `command-code` in headless `-p` print mode with `--trust`, `--skip-onboarding`, `--yolo`, and `--max-turns 30`. It embeds the output schema in the prompt and parses structured JSON from stdout. Pass your own `--max-turns` or `--add-dir` to adjust the turn limit or add extra workspace directories. Token usage is not reported by the CLI.                                                                                                                                                                      |
